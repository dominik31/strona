...
$obraz = $_SESSION['r'];
		...
		try 
		{
			...
			else
			{
				...
				$rezultat = $polaczenie->query("SELECT id FROM min WHERE obraz='$obraz'");
				...
				if ($wszystko_OK==true)
				{
					...
					if ($polaczenie->query("INSERT INTO min (nazwa,opis,data_r,data_z,cena_w,cena_m,obraz) VALUES ('$np', '$opis', '$data_r', '$data_z', '$cena_w', '$cena_m', '$obraz' )"))
					{
						echo 'Uda³o siê dodaæ aukcjê z cen¹ minimaln¹!';
					}
					else
					{
...
CENA MINIMALNA : <input type="number" name="cena_m" min="0"/><br>
<br>
